...
| 包 | 职责 | 重要文件 |
|---|------|---------|
| `controller/` | REST API 入口 | `InterviewController`, `UserController`, `ResumeController`, `QuestionBankAdminController` |
...
- **题库**：PDF/DOCX 等 → `scripts/question_bank_import.py` 生成 JSON 包 → Settings 的 Question Bank Admin 使用 `APP_ADMIN_TOKEN` 审核发布 → MySQL `knowledge_atom` → Qdrant 向量索引
