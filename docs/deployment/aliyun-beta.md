...
INTERWISE_FRONTEND_IMAGE_TAG=20260921-police2
```
...
## 公安备案页脚发布与验收（2026-09-21）

- 正式站点已发布公安备案号 `浙公网安备33020602001742号`、官方图标与对应查询链接，同时保留原 ICP 链接。
- 前端镜像为 `interwise/frontend:20260921-police2`，源码标签为 `9317da30ddb00afae28c823064f0ad95db8feca1`，镜像 ID 为 `sha256:b0ebb82941a92f4da70d2c60c21ee36b3103956d384c1a351e16801cdf619fff`。后端继续使用 `20260918-public1`；后端、MySQL、Redis、Caddy 的容器 ID 均未变化，私有环境配置除两个页脚发布参数外保持不变。
- 前端 30 个测试文件、148 项测试与生产构建通过。公网首页、实际 JS 和图标资源返回 200，应用、MySQL、Redis、Qdrant 健康状态均为 UP。
- 内置浏览器已恢复控制，实际验收正式域名的已登录工作台：1366 像素桌面、375 像素手机和 320 像素窄屏均能显示页脚；窄屏自动换行，图标加载、链接编号、打开方式和布局检查通过。登录页和面试页的备案布局由组件测试覆盖，本次未退出现有账号重复登录或重跑面试。
- 发布检查发现旧默认配置路径触发 Nginx 入口脚本的 `apk manifest` 检查，并在 48 MiB 限制下出现一次启动 OOM。改为独立 `interwise.conf` 后跳过这项多余检查；最终 ECS 容器继续使用 48 MiB 上限，验收时启动峰值约 6.14 MiB，OOM 和重启计数均为 0。该实测只代表此次启动与页面检查，不代表多人面试容量。
- 回滚配置、镜像校验清单和脱敏验收回执保存在服务器发布目录；本次创建的隔离诊断容器已清理，业务数据未迁移或重建。

## 后续步骤

1. **补充页面与普通账号验收。** 正式域名登录和一次面试已由用户手工验收，桌面及移动端备案页脚已完成浏览器检查；继续补查管理员后台以及普通账号不能维护公共题库的完整流程。
