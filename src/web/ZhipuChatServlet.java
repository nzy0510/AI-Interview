...
public class ZhipuChatServlet extends HttpServlet {
    ...
    private String callZhipuAI(String prompt, String jobType) throws IOException {
        ...
        if (responseCode == 200) {
            ...
            try (BufferedReader br = new BufferedReader(
                ...
                System.out.println("【调试】AI 响应内容：" + respSb.toString()); // 打印完整响应
                // 解析 AI 回复
                JSONObject respJson = JSON.parseObject(respSb.toString());
            ...
            try (BufferedReader br = new BufferedReader(
                ...
                return "API 调用失败（状态码：" + responseCode + "）：" + errorSb.toString();
            }
        }
    }
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (jobType == null || jobType.trim().isEmpty()) {
            resp.getWriter().write(JSON.toJSONString(new Result("fail", "请先选择面试岗位！")));
            return;
        }
        if (prompt == null || prompt.trim().isEmpty()) {
            resp.getWriter().write(JSON.toJSONString(new Result("fail", "请输入面试问题！")));
            return;
        }
        ...
        try {
            ...
            resp.getWriter().write(JSON.toJSONString(new Result("success", aiReply)));
        } catch (Exception e) {
            resp.getWriter().write(JSON.toJSONString(new Result("fail", "调用异常：" + e.getMessage())));
        }
    }
